...
public class MealsUtil {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        List<MealTo> mealsTo1 = filteredByCycles(meals, LocalTime.of(7, 0), LocalTime.of(12, 0), MAX_CALORIES);
    ...
    public static List<MealTo> filteredByCycles(List<Meal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        meals.forEach(m -> {
            if (isBetweenInclusive(m.getDateTime().toLocalTime(), startTime, endTime)) {
                mealTo.add(new MealTo(m.getDateTime(), m.getDescription(), m.getCalories(),
                        sumCalByDay.get(m.getDate()) > caloriesPerDay));
            }
        });
        return mealTo;
    }
